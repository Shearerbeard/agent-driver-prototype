...
| `WorkerRoster` | A plan may assign work only to a configured worker, and what the roster advertises for that worker is what the runtime can execute: `from_config` resolves each `mcp_filter` against a caller-supplied `ToolInventory` | A task dispatched to a worker that does not exist, which no later stage could recover from; a roster that advertises tools the runtime does not have, or hides tools it does |
