...
use crate::bounding::{ErrorPreviewWidth, FailureHandleWidth, ToolReasoningWidth};
...
pub struct StructuredTaskOutput {
    pub summary: String,
    pub confidence: crate::tools::submit_result::Confidence,
}
...
pub struct IterationContext {
    ...
    #[serde(default)]
    pub pinned_goal: Option<crate::context::PinnedGoal>,
    ...
    #[serde(default)]
    pub tool_traces: HashMap<usize, Vec<crate::persistence::ToolTraceEntry>>,
...
impl IterationContext {
    ...
    pub fn new(
        ...
        tool_traces: HashMap<usize, Vec<crate::persistence::ToolTraceEntry>>,
    ) -> Self {
        Self {
            iteration,
            previous_plan,
    ...
    #[must_use]
    pub fn with_pinned_goal(mut self, goal: crate::context::PinnedGoal) -> Self {
    ...
    pub fn build_continuation_prompt(
        ...
        use crate::templates::{ContinuationVars, render_continuation_prompt};
        ...
        use crate::prompt_constants::continuation as hdr;
        ...
        let reuse_guidance = if has_failed_tasks && succeeded > 0 {
            crate::prompt_constants::guidance::RESULT_FORWARDING
        } else {
            ""
        };
...
fn artifact_refs(
    traces: Option<&Vec<crate::persistence::ToolTraceEntry>>,
) -> Vec<crate::context::ArtifactRef> {
    use crate::persistence::ToolOutcome;
    ...
    traces
        ...
        .filter_map(|t| match (&t.artifact_filename, &t.outcome) {
            (Some(filename), ToolOutcome::Success { output_bytes }) => {
                crate::context::ArtifactRef::new(filename, *output_bytes).ok()
            }
...
mod tests {
    ...
    use crate::persistence::{ToolOutcome, ToolTraceEntry};
